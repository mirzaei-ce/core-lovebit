...
###   This script attempts to download the signature file SHA256SUMS.asc from lovebit.org
...
WORKINGDIR="/tmp/lovebit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://lovebit.org/bin/"
VERSIONPREFIX="lovebit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'lovebit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Lovebit Core binary release signing key installed?"
   fi
